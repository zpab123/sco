...
// 常量 -- packet 数据大小定义
const (
	_CAP_GROW_SHIFT  uint16 = 2   // 二进制数据 位计算变量
	_MIN_PAYLOAD_CAP uint16 = 128 // buff 最小有效容量（buff 对象池使用）
)
...
var (
	...
	bufPools map[uint16]*sync.Pool = map[uint16]*sync.Pool{}
)

// packet_pool 初始化
func init() {
	// 添加 256 512 2048 ... max 等各个容量数组
	bufCap := uint16(_MIN_PAYLOAD_CAP) << _CAP_GROW_SHIFT
	for bufCap < C_PKT_BODY_MAX_LEN {
...
// 只读不写，所以并发安全
func getPoolKey(need uint16) uint16 {
